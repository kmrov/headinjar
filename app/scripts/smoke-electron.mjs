...
try {
  ...
  assert.equal(await editor.locator('[data-help]').count(), 7, 'only the seven agreed controls get help');
  const helpPopup = editor.locator('#help-popover');
  await editor.locator('[data-help="editor-modes"]').click();
  assert.equal(await helpPopup.isVisible(), true, 'help opens on click');
  assert.match(await helpPopup.innerText(), /digital model|real surface/i);
  assert.equal(await editor.locator('#mode-placement').getAttribute('aria-pressed'), 'true', 'opening mode help does not change mode');
  await editor.locator('[data-help="mapping"]').click();
  assert.match(await helpPopup.innerText(), /Front[\s\S]*Wrap[\s\S]*Surface[\s\S]*Model UV/);
  assert.equal(await editor.locator('[data-help="editor-modes"]').getAttribute('aria-expanded'), 'false', 'opening another help closes the first');
  await editor.keyboard.press('Escape');
  assert.equal(await helpPopup.isVisible(), false, 'Escape closes help');
  ...
  const windowSize = await editor.evaluate(() => ({width: innerWidth, height: innerHeight}));
  assert.ok(helpBounds.x >= 0 && helpBounds.y >= 0 && helpBounds.x + helpBounds.width <= windowSize.width && helpBounds.y + helpBounds.height <= windowSize.height,
    'help stays inside the window beside footer controls');
  await editor.keyboard.press('Escape');
