...
export function createViewport(host, callbacks={}) {
  ...
  function draw(){
    ...
    if(!snapshot || mode!=='placement'||!tool||tool==='place')return;
    if(tool==='align') {
      const pairs=previewPairs??activeMapping(snapshot.project.placement).alignment?.pairs??[];
    ...
    const grid=previewGrid??activeMapping(snapshot.project.placement).grid;
    if(showGrid){
  ...
  function releaseWrapDrag(){
    if(wrapDrag && canvas.hasPointerCapture(wrapDrag.pointerId))canvas.releasePointerCapture(wrapDrag.pointerId);
    wrapDrag=null;
  }
  ...
  canvas.addEventListener('pointerdown',(event)=>{
    if(snapshot&&mode==='projector'&&physical.active){
      ...
      if(nearest?.d<20){physicalDrag={index:nearest.index,point:null};callbacks.onPhysicalSelect?.(nearest.index);canvas.setPointerCapture(event.pointerId);}
      return;
    }
    ...
    if(event.button===1){
      ...
      if(snapshot.project.mesh&&scene?.beginPan(event.clientX,event.clientY)){
        alignmentPan={x:event.clientX,y:event.clientY};canvas.setPointerCapture(event.pointerId);
      }
      return;
    }
    if(event.button!==0)return;
    if(tool==='place'){
      ...
      if(snapshot.project.placement.mappingMode!=='surface')return;
    ...
    if(tool==='align') {
      ...
      const pairs=activeMapping(snapshot.project.placement).alignment?.pairs??[];
      ...
      if(nearest?.d<16){
        ...
        callbacks.onAlignmentSelection?.(nearest.index);canvas.setPointerCapture(event.pointerId);draw();
      }else {
    ...
    const nearest=activeMapping(snapshot.project.placement).grid.points.map((p,index)=>({index,d:Math.hypot(xy(p).x-x,xy(p).y-y)})).sort((a,b)=>a.d-b.d)[0];
  ...
  canvas.addEventListener('pointermove',(event)=>{
    ...
    if(alignmentDrag){
      ...
      try{previewGrid=moveGridPoint(activeMapping(snapshot.project.placement).grid,drag,uv(event));
        const placement=snapshot.project.placement;
        scene?.previewPlacement(placement.mappingMode==='wrap'
          ? {...placement,wrap:{...placement.wrap,grid:previewGrid}} : {...placement,grid:previewGrid});draw();
  ...
  canvas.addEventListener('pointerup',(event)=>{
    ...
    if(wrapDrag?.pointerId===event.pointerId){
      const transform=wrapDrag.latest;releaseWrapDrag();
      if(transform)Promise.resolve(callbacks.onWrapTransform?.(transform)).catch(report).finally(clearPreview);
      else clearPreview();
      return;
    }
  ...
  return {
    setSnapshot(value){
      if(snapshot?.project.id!==value.project.id || snapshot?.project.revision!==value.project.revision){
        drag=null;alignmentDrag=null;releaseSurfaceDrag();releaseWrapDrag();surfacePreview=null;
      }
      ...
      if(hasMesh){
        try{scene??=createScenePreview(canvas,callbacks.onError??(()=>{}),{onViewChange:draw});scene.setVideoSourceRequired(value.source?.kind==='webrtc');scene.setSnapshot(value);scene.setMode(mode);scene.setTextureOpacity(textureOpacity);
    ...
    getPhysicalLandmarks(){
      ...
      return pairs.map(pair=>scene?.projectDomainNormalized(pair.target)).filter(Boolean);
    },
    ...
    previewAlignment(pairs){
      previewPairs=pairs;
      if(pairs.length>=3){
        try{const grid=fitLandmarkGrid(pairs),placement=snapshot.project.placement,transform={x:0,y:0,scale:1,rotation:0};
          scene?.previewPlacement(placement.mappingMode==='wrap'
            ? {...placement,wrap:{...placement.wrap,grid,transform}} : {...placement,grid,transform});}
